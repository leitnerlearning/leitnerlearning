...
3. Thematic mini-packs (Airport, Café, Campus, Doctor, Housing, Work, Shopping) — **opt-in** from Library → Themes; never auto-dump into day one. Enabled packs merge unique cards (ranks 1001+) and get a small daily intro bias after the hook phase. Library filter **Themes** lists pack cards grouped by pack. Progress shows a calm **Themes** glance. **Study** starts a short focused Review set for that pack (cap ~12).  
